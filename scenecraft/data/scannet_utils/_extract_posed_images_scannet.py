...
def process_scene(path: str, output_path: str, scene_id: str, limit: int) -> None:
    """Process single ScanNet scene.

    Extract RGB images, poses and camera intrinsics.
    """
    print(f"Processing {scene_id}")
    data = SensorData(os.path.join(path, scene_id, f'{scene_id}.sens'), limit)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--path', type=str, default='/mnt/data2/scannet/scans')
    parser.add_argument('--output_path', type=str, default='/mnt/data2/scannet/posed_images')
    parser.add_argument('--max-images-per-scene', type=int, default=600)
    parser.add_argument('--nproc', type=int, default=16)
    args = parser.parse_args()
    split = args.path.split('/')[-1]
    scenes = os.listdir(args.path)
    print(f'Processing {split}, found {len(scenes)} scenes ...')
    paths = [args.path] * len(scenes)
    output_paths = [args.output_path] * len(scenes)
    max_images_per_scene = [args.max_images_per_scene] * len(scenes)
    with futures.ProcessPoolExecutor(max_workers=args.nproc) as executor:
        executor.map(process_scene, paths, output_paths, scenes, max_images_per_scene)
    print('Done!')
